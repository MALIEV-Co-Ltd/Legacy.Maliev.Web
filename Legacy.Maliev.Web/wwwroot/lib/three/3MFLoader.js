	...
	class ThreeMFLoader extends THREE.Loader {
		...
		parse( data ) {
			...
			function getBuild( data, objects, modelData, textureData, objectData, builder, modelsData ) {
				...
				data.build = builder( data, objects, modelData, textureData, objectData, modelsData );
				return data.build;
			...
			function buildObject( objectId, objects, modelData, textureData, modelsData ) {
				...
				// Production 3MF packages may put component meshes in a separate model
				// part. The root model is commonly listed before that external part, so
				// resolving only against the current model makes a valid component look
				// like an undefined object and throws while reading its mesh.
				if ( objectData === undefined && modelsData ) {

					const modelKeys = Object.keys( modelsData );
					for ( let i = 0; i < modelKeys.length; i ++ ) {

						const candidateModelData = modelsData[ modelKeys[ i ] ];
				...
				if ( objectData[ 'mesh' ] ) {
					...
					objects[ objectData.id ] = getBuild( meshData, objects, modelData, textureData, objectData, buildGroup, modelsData );

				} else {

					const compositeData = objectData[ 'components' ];
					objects[ objectData.id ] = getBuild( compositeData, objects, modelData, textureData, objectData, buildComposite, modelsData );
