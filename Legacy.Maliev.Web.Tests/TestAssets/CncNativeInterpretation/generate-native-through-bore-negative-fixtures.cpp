// Independent valid two-bore counterexample: both holes must remain owned; one-bore target must reject.
...
int main(int argc,char** argv){try{
    ...
    if(family!="two-bores" || (variant!="normal" && variant!="placed"))throw std::runtime_error("unknown fixture");
    for(double x:{11.0,23.0})shape=BRepAlgoAPI_Cut(shape,BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(x,13,4),gp::DZ()),3,14).Shape()).Shape();
    ...
    const std::string name="native-through-bore-negative-"+family+(variant=="normal"?"":"-"+variant);
    ...
    std::cout<<name<<" native-valid=true target=30x20x12 two-disjoint-bores-radius=3 centers-x=11,23"<<"\n";return 0;
