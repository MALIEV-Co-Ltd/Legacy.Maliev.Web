...
int main(int argc,char** argv) { try {
  std::string variant=argc>1?argv[1]:"thread-external-rh";
  hand=variant.find("-lh")!=std::string::npos?-1:1;
  bool placed=variant.find("placed")!=std::string::npos,connector=variant.find("connector")!=std::string::npos;
  reverseAxis=reverseEdges=reverseU=variant.find("reversed")!=std::string::npos;
  connectorMode=connector;
  if(connector){
    std::cerr<<"stage lower thread"<<std::endl;
    thread(0,2);sides.push_back(cap(0));sides.push_back(annularCap(1,lowR));
    std::cerr<<"stage upper thread"<<std::endl;
    thread(3,2);sides.push_back(annularCap(0,highR));sides.push_back(cap(1));
    // Direct polygon unions in the exact cylindrical parameter planes. The
    // circular connector boundaries and separate helical boundaries now belong
    // to the same finite native faces without global same-domain unification.
    for(auto it=extendedBands.rbegin();it!=extendedBands.rend();++it)sides.erase(sides.begin()+*it);
    clipLow=0;clipHigh=5;double rise=(highR-lowR)/std::sqrt(3.0),a=1+halfCrest+rise,b=2-halfCrest-rise;
    Handle(Geom_Surface) lower=new Geom_CylindricalSurface(gp_Ax3(gp::Origin(),gp::DZ()),lowR);
    std::vector<UV> lowerTrim={{0,a},{2*pi*(2-a),2},{2*pi,2},{2*pi,2.25},{0,2.25},{0,2},{2*pi*(2-b),2},{0,b}};
    face(lower,lowerTrim,0,1,0);
    Handle(Geom_Surface) upper=new Geom_CylindricalSurface(gp_Ax3(gp::Origin(),gp::DZ()),highR);
    std::vector<UV> upperTrim={{0,2.25+rise},{2*pi,2.25+rise},{2*pi,3+halfCrest},{2*pi*(1-halfCrest),3},
      {2*pi*halfCrest,3},{2*pi,4-halfCrest},{2*pi,4+halfCrest},{0,3+halfCrest}};
    face(upper,upperTrim,0,1,0);
    flank(2.25,2.25+rise,0);
  }else{thread(0,4);sides.push_back(cap(0));sides.push_back(cap(1));}
  BRepBuilderAPI_Sewing sew(1e-5);for(auto f:sides)sew.Add(f);
  std::cerr<<"stage sewing"<<std::endl;
  sew.Perform(); TopExp_Explorer shells(sew.SewedShape(),TopAbs_SHELL);
  if(!shells.More()) throw std::runtime_error("no sewn shell");
  auto solid=BRepBuilderAPI_MakeSolid(TopoDS::Shell(shells.Current())).Solid();shells.Next();
  std::cerr<<"stage solid validation"<<std::endl;
  if(shells.More() || !BRepLib::OrientClosedSolid(solid) || !BRepCheck_Analyzer(solid).IsValid()) throw std::runtime_error("invalid sewn solid");
  TopoDS_Shape shape=solid;
  if(placed){gp_Trsf transform;transform.SetRotation(gp_Ax1(gp::Origin(),gp_Dir(1,2,3)),.713);transform.SetTranslationPart(gp_Vec(17,-23,41));shape=BRepBuilderAPI_Transform(shape,transform,true).Shape();}
  GProp_GProps mass;BRepGProp::VolumeProperties(solid,mass);std::cout<<"public thread volume "<<mass.Mass()<<std::endl;
  ...
  std::string name="/"+variant+".step";std::ofstream output(name);if(!precise.Print(output)) throw std::runtime_error("write failed");output.close();
  EM_ASM({const n=UTF8ToString($0);require('fs').writeFileSync('/tmp'+n,FS.readFile(n));},name.c_str());
