...
TopoDS_Shape roof(bool reversed) {
 ...
 for(TopExp_Explorer ex(top,TopAbs_EDGE);ex.More();ex.Next()){
  TopoDS_Edge e=TopoDS::Edge(ex.Current());TopoDS_Vertex a,b;TopExp::Vertices(e,a,b,true);
 ...
 for(int i=0;i<4;++i)bottom.Add(BRepBuilderAPI_MakeEdge(p[i],p[(i+1)%4]));
...
int main(int argc,char** argv){
 ...
 if(family=="rational-roof")shape=roof(reverse);
 else if(family=="thread-small-sphere"){
  ...
  auto sphere=BRepPrimAPI_MakeSphere(gp_Ax2(gp_Pnt(3,0,4.25),reverse?-gp::DX():gp::DX(),gp::DY()),.5).Shape();
  shape=BRepAlgoAPI_Cut(reader.OneShape(),sphere).Shape();
 }
 else {
